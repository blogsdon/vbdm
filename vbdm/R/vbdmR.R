...
update_p <- function(model){
	...
	for(j in 1:model$m){
		...
		#a3 <- -log(model$prob);
		#equation 10
		a3 <- -(digamma(model$prob*(model$m+4))-digamma(model$m+4));
		#cat(a3,'a3\n')
		...
		#a4 <- log(1-model$prob);
		#equation 10
		a4 <- (digamma((1-model$prob)*(model$m+4))-digamma(model$m+4))
		#cat(a4,'a4\n')
	...
	#equation 17/18
	model$prob <- (sum(model$pvec)+2)/(model$m+4);
	#cat(model$prob,'prob\n');
...
update_lb <- function(model){
	...
	alpha1 <- sum(model$pvec)+2;
	beta1  <- model$m-sum(model$pvec)+2;
...
run_discrete_mixture_vanilla <- function(y,G,X=NULL,tol=1e-4,theta=NULL,thres=0.05,scaling=TRUE){
	model <- list();
...
discrete.mixture.test <- function(y,G,X=NULL,tol=1e-4,thres=0.05,scaling=TRUE){
	model_a <- run_discrete_mixture_vanilla(y=y,G=G,X=X,tol=tol,thres=thres,scaling=scaling);
	model_b <- run_discrete_mixture_vanilla(y=y,G=G,X=X,tol=tol,theta=0,thres=thres,scaling=scaling);
